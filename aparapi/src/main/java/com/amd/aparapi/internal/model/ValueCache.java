...
public final class ValueCache<K, V, T extends Throwable> {
    ...
    //    @FunctionalInterface
    public interface ValueComputer<K, V> extends
        ThrowingValueComputer<K, V, RuntimeException> {
        // Marker interface
    }

    public static <K, V, T extends Throwable> ValueCache<K, V, T> on(
        ThrowingValueComputer<K, V, T>
        computer) {
        return new ValueCache<K, V, T>(computer);
    }

    private final ConcurrentMap<K, SoftReference<V>> map = new
    ConcurrentHashMap<>();
