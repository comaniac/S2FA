...
public abstract class ClassModel {
	...
	public class ConstantPool implements Iterable<ConstantPool.Entry> {
		...
		public abstract class MethodReferenceEntry extends ReferenceEntry {
			...
			public int hashCode() {
				...
				return ((((nameAndTypeEntry.getNameIndex() * 31) + nameAndTypeEntry.getDescriptorIndex()) * 31) +
				        getClassIndex());
			}
		...
		public String getDescription(ConstantPool.Entry _entry) {
			...
			if (_entry instanceof ConstantPool.EmptyEntry)
				...
				final ConstantPool.UTF8Entry utf8Entry = (ConstantPool.UTF8Entry) get(classEntry.getNameIndex());
				final ConstantPool.NameAndTypeEntry nameAndTypeEntry = (ConstantPool.NameAndTypeEntry) get(
				      methodEntry
				      .getNameAndTypeIndex());
				final ConstantPool.UTF8Entry utf8NameEntry = (ConstantPool.UTF8Entry) get(
				      nameAndTypeEntry.getNameIndex());
				...
				final ConstantPool.InterfaceMethodEntry interfaceMethodEntry = (ConstantPool.InterfaceMethodEntry)
				    _entry;
				final ConstantPool.ClassEntry classEntry = (ConstantPool.ClassEntry) get(
		...
		public AttributePool(ByteReader _byteReader, String name) {
			...
			for (int i = 0; i < attributeCount; i++) {
				...
				if (attributeName.equals(LOCALVARIABLETABLE_TAG)) {
					...
					runtimeInvisibleAnnotationsEntry = new RuntimeAnnotationsEntry(_byteReader, attributeNameIndex,
					    length);
					entry = runtimeInvisibleAnnotationsEntry;
				} else if (attributeName.equals(RUNTIMEVISIBLEANNOTATIONS_TAG)) {
					runtimeVisibleAnnotationsEntry = new RuntimeAnnotationsEntry(_byteReader, attributeNameIndex,
					    length);
	...
	public ClassModelMethod getMethod(MethodEntry _methodEntry, boolean _isSpecial) {
		...
		ClassModelMethod methodOrNull = getMethodOrNull(nameAndTypeEntry.getNameUTF8Entry().getUTF8(),
		                                nameAndTypeEntry
