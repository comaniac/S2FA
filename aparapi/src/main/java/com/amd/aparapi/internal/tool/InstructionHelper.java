...
public class InstructionHelper {
    ...
    public static class StringWriter extends BlockWriter {
        ...
        @Override public void writeConstructorCall(ConstructorCall call) throws
            CodeGenException {
            // TODO
        }
        ...
        @Override public void writeMethodBody(MethodModel _methodModel) throws
            CodeGenException {
    ...
    public static class BranchVector {
        ...
        public boolean overlaps(BranchVector _other) {
            final boolean overlap = ((start.getThisPC() < _other.start.getThisPC()) &&
                                     (end.getThisPC() > _other.start.getThisPC()) && (end
                                             .getThisPC() <= _other.end.getThisPC())) //
                                    || ((_other.start.getThisPC() < start.getThisPC()) &&
                                        (_other.start.getThisPC() > start.getThisPC()) && (_other.end
        ...
        @Override public boolean equals(Object other) {
            return ((other instanceof BranchVector) && ((other == this) ||
                    (((BranchVector) other).from
    ...
    public static String getLabel(Instruction instruction, boolean showNumber,
        ...
        if (!showExpressions) {
            ...
            if (!verboseBytecodeLabels)
                ...
                if (instruction instanceof ConditionalBranch16) {
                    final ConditionalBranch16 conditionalBranch16 = (ConditionalBranch16)
                            instruction;
                    ...
                    label.append(
                        methodCall.getConstantPoolMethodEntry().getNameAndTypeEntry().getNameUTF8Entry().getUTF8());
                    label.append(" ");
                    label.append(
                        methodCall.getConstantPoolMethodEntry().getNameAndTypeEntry().getDescriptorUTF8Entry().getUTF8());
                } else if (instruction instanceof OperatorInstruction) {
                    final OperatorInstruction operatorInstruction = (OperatorInstruction)
                            instruction;
                    ...
                    label.append(
                        field.getConstantPoolFieldEntry().getNameAndTypeEntry().getNameUTF8Entry().getUTF8());
                    label.append(
                        field.getConstantPoolFieldEntry().getNameAndTypeEntry().getDescriptorUTF8Entry().getUTF8());
                    ...
                    final LocalVariableTableIndexAccessor localVariableAccessor =
                        (LocalVariableTableIndexAccessor)
                        instruction;
                    ...
                    label.append(" " + ((I_IINC)
                                        instruction).getLocalVariableInfo().getVariableName());
    ...
    private static void appendFoldedInstruction(Table _sl, String _prefix,
    ...
    static void writeExpression(String _prefix, Instruction _instruction) {
        System.out.println(_prefix + InstructionHelper.getLabel(_instruction, true,
                           true, false));
    }
    ...
    static String getFoldedView(MethodModel _methodModel) {
        ...
        for (Instruction root = _methodModel.getExprHead(); root != null;
                root = root.getNextExpr())
            appendFoldedInstruction(sl, "", root);
        return (sl.toString());
    }

    static String createView(MethodModel _methodModel, String _msg,
                             Instruction _head) {
    ...
    private static Comparator<BranchVector> branchInfoComparator = new
    Comparator<BranchVector>() {
    ...
    static List<BranchVector> getBranches(MethodModel _methodModel) {
        ...
        for (Instruction instruction = _methodModel.getPCHead(); instruction != null;
    ...
    void fdump(int _depth, Instruction i, boolean clone) {
        final String label =
            i.getByteCode().getName();// InstructionHelper.getLabel(i, false, false, false);
