...
public abstract class CustomizedClassModel extends ClassModel {
    ...
    public List<FieldDescriptor> getStructMemberInfo() {
        ...
        int id = 0;
        for (CustomizedFieldModel f : fields) {
            members.add(new FieldDescriptor(id, f.getTypeSpec(), f.getName(),
                                            f.getOffset()));
            id += 1;
        }
        return members;
    ...
    public static class TypeParameters implements Comparable<TypeParameters>,
        Iterable<String> {
