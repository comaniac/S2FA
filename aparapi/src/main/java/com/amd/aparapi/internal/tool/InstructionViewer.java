...
public class InstructionViewer implements Config.InstructionListener {
	...
	public static abstract class Form<T extends Form.Template> {
		...
		public Form(T _template) {
			...
			for (final Field field : fieldToLabelMap.keySet()) {
				...
				layout.putConstraint(SpringLayout.WEST, fieldToLabelMap.get(field), INSET, SpringLayout.WEST,
				                     panel);
	...
	public void fillStroke(Graphics2D _g, Color _fillColor, Color _strokeColor, Stroke _stroke,
	                       Shape _rect) {
		final Color color = _g.getColor();
	...
	private final Map<Instruction, InstructionView> locationToInstructionViewMap = new
	...
	double foldPlace(Graphics2D _g, InstructionView _instructionView, double _x, double _y,
	                 boolean _dim) {
		...
		if (!config.collapseAll && !config.showExpressions) {
			...
			if (config.edgeFan) {
				...
				if ((instruction.getFirstChild() != null) &&
					...
					for (Instruction e = instruction.getFirstChild(); e != null; e = e.getNextExpr()) {
						final InstructionView iv = getInstructionView(e);
						line(_g, midx, iv.shape.getBounds().getCenterY(), iv.shape.getBounds().getMinX() - ARROWGAP,
						     iv.shape.getBounds()
						     .getCenterY());
					}
	...
	public void edge(Graphics2D _g, Color _color, InstructionView _branch, InstructionView _target,
	                 String _endLabel,
	                 String _startLabel) {
		...
		if (_target.shape != null) {
			...
			if (delta > 0) {
				...
				_g.draw(new CubicCurve2D.Double(x1, y1, Math.max(x1, x2) + adjust, y1, Math.max(x1, x2) + adjust,
				                                y2, x2, y2));
				final AffineTransform transform = _g.getTransform();
	...
	public static void main(String[] _args) throws ClassNotFoundException, InstantiationException,
