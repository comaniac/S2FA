...
public class InstructionViewer implements Config.InstructionListener {
    ...
    public static abstract class Form<T extends Form.Template> {
        ...
        public Form(T _template) {
            ...
            for (final Field field : fieldToLabelMap.keySet()) {
                layout.putConstraint(SpringLayout.NORTH, fieldToLabelMap.get(field), INSET,
                                     (last == panel) ? SpringLayout.NORTH
                                     : SpringLayout.SOUTH, last);
                layout.putConstraint(SpringLayout.WEST, fieldToLabelMap.get(field), INSET,
                                     SpringLayout.WEST,
                                     panel);
                ...
                if (field.getType().isAssignableFrom(Boolean.TYPE)) {
                    ...
                    if (toggleAnnotation != null) {
                        ...
                        final String toggleButtonLabel = getBoolean(field) ? toggleButtonOnLabel :
                                                         toggleButtonOffLabel;
                        final JToggleButton toggleButton = new JToggleButton(toggleButtonLabel,
                                getBoolean(field));
                ...
                if (newComponent != null) {
                    ...
                    layout.putConstraint(SpringLayout.NORTH, newComponent, INSET,
                                         (last == panel) ? SpringLayout.NORTH
                                         : SpringLayout.SOUTH, last);
                    layout.putConstraint(SpringLayout.WEST, newComponent, INSET, SpringLayout.EAST,
                                         fieldToLabelMap.get(fieldWithWidestLabel));
                    layout.putConstraint(SpringLayout.EAST, newComponent, INSET, SpringLayout.EAST,
                                         panel);
                }
                last = newComponent;
            }
    ...
    private static class View {
        ...
        public double translatex(int _screenx) {
            return ((_screenx - offGraphicsTransform.getTranslateX()) /
                    offGraphicsTransform.getScaleX());

        }

        public double screenx() {
            return ((offGraphicsTransform.getScaleX() * x) +
                    offGraphicsTransform.getTranslateX());
        }

        public double translatey(int _screeny) {
            return ((_screeny - offGraphicsTransform.getTranslateY()) /
                    offGraphicsTransform.getScaleY());
        }

        public double screeny() {
            return ((offGraphicsTransform.getScaleY() * y) +
                    offGraphicsTransform.getTranslateY());
        }
    }
    ...
    public synchronized void draw(Graphics _g) {
        ...
        if (dirty || (offscreen == null) || (containerSize.width != offscreensize.width)
            ...
            offscreen = (BufferedImage) container.createImage(offscreensize.width,
            ...
            offgraphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
            ...
            offgraphics.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS,
    ...
    public void text(Graphics2D _g, Color _color, String _text, double _x,
                     double _y) {
    ...
    public void line(Graphics2D _g, Stroke _stroke, double _x1, double _y1,
    ...
    public void fillStroke(Graphics2D _g, Color _fillColor, Color _strokeColor,
                           Stroke _stroke,
                           Shape _rect) {
        final Color color = _g.getColor();
    ...
    public void line(Graphics2D _g, double _x1, double _y1, double _x2,
                     double _y2) {
    ...
    private final Map<Instruction, InstructionView> locationToInstructionViewMap =
        new
    HashMap<Instruction, InstructionView>();

    InstructionView getInstructionView(Instruction _instruction) {

        InstructionView instructionView = locationToInstructionViewMap.get(
                                              _instruction);
        if (instructionView == null)
            locationToInstructionViewMap.put(_instruction,
                                             instructionView = new InstructionView(_instruction));

        return (instructionView);
    }

    double foldPlace(Graphics2D _g, InstructionView _instructionView, double _x,
                     double _y,
                     boolean _dim) {
        _instructionView.dim = _dim;
        final FontMetrics fm = _g.getFontMetrics();

        _instructionView.label = InstructionHelper.getLabel(
                                     _instructionView.instruction, config.showPc,
        ...
        double y = _y;
        final double x = _x + w + (_instructionView.instruction.getRootExpr() ==
                                   _instructionView.instruction ? HGAP : HGAP);

        if (!config.collapseAll && !config.showExpressions) {

            for (Instruction e = _instructionView.instruction.getFirstChild(); e != null;
                    e = e.getNextExpr()) {
    ...
    void foldRender(Graphics2D _g, InstructionView _instructionView) {
        ...
        if (!config.collapseAll && !config.showExpressions) {
            for (Instruction e = instruction.getFirstChild(); e != null;
                    e = e.getNextExpr())
        ...
        text(_g, _instructionView.label, _instructionView.shape.getBounds().getCenterX()
             - (_instructionView.shape.getBounds().getWidth() / 2),
             _instructionView.shape.getBounds().getCenterY());
        ...
        if (!config.collapseAll && !config.showExpressions) {
            ...
            if (config.edgeFan) {
                ...
                for (Instruction e = instruction.getFirstChild(); e != null;
                    ...
                    if (config.edgeCurve)
                        _g.draw(new CubicCurve2D.Double(x1, y1, x1 + CURVEBOW, y1, x2 - CURVEBOW, y2,
                                                        x2, y2));
                    else {
                ...
                if ((instruction.getFirstChild() != null) &&
                        (instruction.getFirstChild() != instruction.getLastChild())) { // >1 children
                    ...
                    final double midx = (_instructionView.shape.getBounds().getMaxX() +
                                         iv0.shape.getBounds().getMinX()) / 2;
                    line(_g, midx, iv0.shape.getBounds().getCenterY(), midx,
                         ivn.shape.getBounds().getCenterY());
                    line(_g, _instructionView.shape.getBounds().getMaxX() + ARROWGAP,
                         _instructionView.shape.getBounds().getCenterY(),
                    ...
                    for (Instruction e = instruction.getFirstChild(); e != null;
                            e = e.getNextExpr()) {
                        final InstructionView iv = getInstructionView(e);
                        line(_g, midx, iv.shape.getBounds().getCenterY(),
                             iv.shape.getBounds().getMinX() - ARROWGAP,
                             iv.shape.getBounds()
                             .getCenterY());
                    }
    ...
    double flatPlace(Graphics2D _g, InstructionView _instructionView, double _x,
                     double _y) {
        final FontMetrics fm = _g.getFontMetrics();
        final Instruction instruction = _instructionView.instruction;
        ...
        _instructionView.shape = new Rectangle((int) _x, (int) top,
                                               fm.stringWidth(_instructionView.label) + 2, h);
        return (_y + h);
    }
    ...
    public void render(Graphics2D _g) {
        if (first != null) {
            ...
            if (config.fold) {
                ...
                for (Instruction instruction = firstRoot; instruction != null;
                ...
                for (Instruction instruction = lastInstruction.getNextPC(); instruction != null;
                ...
                for (final InstructionView instructionView : instructionViews) {
                    if (instructionView.instruction.isBranch()) {
                        ...
                        final Instruction rootToInstruction =
                            instructionView.instruction.asBranch().getTarget();
                ...
                for (final InstructionView instructionView : instructionViews) {
                    ...
                    if (last != null)
                        line(_g, thickStroke, 120, last.shape.getBounds().getMaxY(), 120,
                             instructionView.shape.getBounds().getMinY());
                    foldRender(_g, instructionView);
    ...
    public void edge(Graphics2D _g, Color _color, InstructionView _branch,
                     InstructionView _target,
                     String _endLabel,
                     String _startLabel) {

        final int delta = _target.instruction.getThisPC() -
                          _branch.instruction.getThisPC();
        ...
        if (_target.shape != null) {
            ...
            if (delta > 0) {
                ...
                _g.draw(new CubicCurve2D.Double(x1, y1, x1 - adjust, y1, x1 - adjust, y2, x2,
                                                y2));
                ...
                _g.draw(new CubicCurve2D.Double(x1, y1, Math.max(x1, x2) + adjust, y1,
                                                Math.max(x1, x2) + adjust,
                                                y2, x2, y2));
                final AffineTransform transform = _g.getTransform();
    ...
    @Override public void showAndTell(String message, Instruction head,
                                      Instruction _instruction) {
    ...
    public static void main(String[] _args) throws ClassNotFoundException,
        ...
        final InstructionViewer instructionViewer = new InstructionViewer(background,
                _args[0]);
