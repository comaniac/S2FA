...
public:
  ...
  virtual void compute() {
	    ...
	    int data_length = getInputLength(0);
			int item_length = 6;
			int ref_length = getInputLength(1);

	    // get the pointer to input/output data
			cl_mem data = *((cl_mem*)getInput(0));
			cl_mem ref_data = *((cl_mem*)getInput(1));
			...
			err |= clSetKernelArg(kernel, 4, sizeof(int), &item_length);
			err |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &ref_data);	
			err |= clSetKernelArg(kernel, 6, sizeof(int), &ref_length);
			if (err != CL_SUCCESS)
