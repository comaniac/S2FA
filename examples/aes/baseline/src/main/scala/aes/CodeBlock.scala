...
class CodeBlock(
  ...
  override def getTypeSize = 32

  override def getPrimitiveData = pData

  override def serialize: Unit = {
    val buf = openBuffer.asCharBuffer
    this.data.foreach(e => {
      buf.put(e.data.asInstanceOf[Array[Char]])
    })
    closeBuffer
  }

  override def deserialize: Unit = {
    val flatData = new Array[Char](numElt * 16)
    val buf = openBuffer.asCharBuffer

    buf.get(this.data.asInstanceOf[Array[Char]], 0, numElt * 16)
