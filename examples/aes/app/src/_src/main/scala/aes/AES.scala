...
object AES {
  ...
  def main(args: Array[String]) {
    ...
    val rdd = acc.wrap(sc.textFile(filePath).map(line => {
      new Code(line.split(" ").map(e => e.toInt))
    }))
    ...
    println("First result: " + firstResult.data.mkString)

    acc.stop
  }
}

class AES(bKey: org.apache.spark.blaze.BlazeBroadcast[Array[Int]]) 
  ...
  def sbox(idx: Int): Int = {
    val inbox = new Array[Int](256)
    inbox(0) = 0x63
    inbox(1) = 0x7c
    inbox(2) = 0x77
    inbox(3) = 0x7b
...
*/
  ...
  override def call(in: Code): Code = {
    ...
    while (i > 0) {
      ...
      deckey(0) = deckey(0) ^ sbox(29) ^ rcon
      deckey(1) = deckey(1) ^ sbox(30)
      deckey(2) = deckey(2) ^ sbox(31)
      deckey(3) = deckey(3) ^ sbox(28)
    ...
    while (i < 14) {
      ...
      j = 15
      while (j > 0) {
        aes_data(j) = sbox(aes_data(j) % 256)
        j -= 1
      }
      ...
      while (j < 16) {
        ...
        var d = aes_data(j + 3)
        var e = a ^ b ^ c ^ d
        aes_data(j) ^= e ^ rj_xtime(a ^ b)
        aes_data(j + 1) ^= e ^ rj_xtime(b ^ c)
        aes_data(j + 2) ^= e ^ rj_xtime(c ^ d)
        aes_data(j + 3) ^= e ^ rj_xtime(d ^ a)
        j += 4
      }
    ...
    i = 15
    while (i > 0) {
      aes_data(i) = sbox(aes_data(i) % 256)
      i -= 1
    }
...
class Code(val data: Array[Int]) extends Serializable { }
