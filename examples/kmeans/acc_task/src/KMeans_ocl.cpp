  ...
  virtual void compute() {
	    ...
	    if (!buf_data || !buf_centers) {
	  	...
	  	// get the pointer to input/output data
			int num_points = data_length / dims;
	    ...
	    cl_mem closest_centers = *((cl_mem*)getOutput(0, 1, num_points, sizeof(int)));
			...
			err |= clSetKernelArg(kernel, 3, sizeof(int), &dims);
			err |= clSetKernelArg(kernel, 4, sizeof(int), &dims);
			err |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &buf_centers);
			err |= clSetKernelArg(kernel, 6, sizeof(int), &center_length);
			if (err != CL_SUCCESS) {
